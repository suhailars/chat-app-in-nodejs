...
var MainController = function() {
    ...
    self.viewEventBus.on("login", function(name, room) {
	// socketio login
	self.chatClient.login(name, room);
    });
    ...
    self.appEventBus.on("loginDone", function(data) {
	self.homeModel = new HomeModel();
        self.homeModel.addChat({sender: '', message: data });
